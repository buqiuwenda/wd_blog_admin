        ...
        <div class="col-lg-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <h5>新增节点 <small> <span class="text-danger">[*]</span>为必填项</small></h5>
                </div>
                @include('errors.error')
                <div class="ibox-content">
                    <form method="post" class="form-horizontal" action="{{ route('node.store') }}">
                        @csrf
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label class="col-sm-2 control-label">节点名称<small class="text-danger">[*]</small></label>
                            <div class="col-sm-4">
                                <input type="text" class="form-control" name="name" placeholder="节点名称，如：用户管理" value="{{ old('name') }}" maxlength="128">
                            </div>
                            ...
                            <div class="col-sm-4">
                                <select name="parent_id" class="form-control m-b">
                                    ...
                                    @foreach($nodes as $node)
                                        <option @if(old('parent_id') == $node->id) selected @endif value="{{ $node->id }}">{{ $node->name }}</option>
                                    @endforeach
                                    @endif
                                </select>
                            </div>
                        ...
                        <div class="form-group {{ $errors->has('routing') ? 'has-error' : '' }}">
                            <label class="col-sm-2 control-label">路由别名 <small class="text-danger">[*]</small></label>
                            <div class="col-sm-4">
                        ...
                        <div class="form-group {{ $errors->has('memo') ? 'has-error' : '' }}">
                            ...
                            <div class="col-sm-4"><textarea name="memo" rows="5"  class="form-control "> {{ old('memo') }}</textarea></div>
                            @if ($errors->has('memo'))
                                <span class="help-block">
